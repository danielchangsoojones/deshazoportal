...
export default function SteelPhotoUpload() {
  const [user, setUser] = useState<User | null>(null)
  const [authLoading, setAuthLoading] = useState(true)
  const [mediaUrl, setMediaUrl] = useState('')
  const [mediaType, setMediaType] = useState<'image' | 'pdf' | ''>('')
  ...
  const uploadFile = async (files: FileList | null) => {
    ...
    if (!isPdf && !isImage) {
      setMessage('Choose an image or PDF file.')
      return
    }
  ...
  return (
    <div className="min-h-screen bg-[#e8eaef] text-[#111]">
      <header className="sticky top-0 z-30 flex h-14 items-center justify-between bg-[linear-gradient(90deg,#3cb9c5_0%,#7a35e8_100%)] px-4 text-white shadow-sm">
        ...
        <div className="text-sm font-black tracking-wide">Steel Dimension Extractor</div>
        ...
        <div className="flex items-center gap-2">
          ...
          >
            {extracting ? 'Loading...' : 'Upload Photo'}
          </button>
          <button
            type="button"
      ...
      <main className="grid h-[calc(100vh-56px)] min-h-0 grid-cols-1 overflow-hidden bg-[#f3f4f8] lg:grid-cols-[minmax(420px,1fr)_620px]">
        <section className="min-h-0 border-r border-[#d9dce5] bg-[#20242c]">
          {mediaUrl && mediaType === 'pdf' ? (
            ...
            <div className="flex h-full items-center justify-center overflow-auto bg-[#20242c] p-6">
              <img
                src={mediaUrl}
                alt={fileName || 'Uploaded steel drawing'}
                className="max-h-full max-w-full rounded-md bg-white object-contain shadow-[0_24px_70px_-40px_rgba(0,0,0,0.8)]"
              />
            </div>
          ) : (
            <div className="flex h-full items-center justify-center px-6 text-center">
              <div className="max-w-sm rounded-md border border-dashed border-white/25 bg-white/10 px-6 py-8 text-white">
                <p className="text-lg font-black">Upload a steel photo</p>
                <p className="mt-2 text-sm font-semibold text-white/70">
                  The drawing or part image preview will appear here.
                </p>
                <button
                  type="button"
                  onClick={() => fileInputRef.current?.click()}
        ...
        <aside className="min-h-0 overflow-y-auto bg-white">
          <div className="sticky top-0 z-10 border-b border-[#dfe4ef] bg-white px-5 py-4">
            ...
            <p className="mt-1 text-[13px] font-semibold text-[#747b8a]">
              Fill in the steel takeoff fields, then use the preview to verify each value.
            </p>
          </div>
          ...
          <div className="space-y-4 px-5 py-5">
            ...
            {fileName ? (
              <section className="rounded-md border border-[#dfe4ef] bg-[#fbfcff] p-4">
                <h2 className="text-[13px] font-black uppercase text-[#273f7a]">Upload Info</h2>
            ...
            <section className="overflow-hidden rounded-md border border-[#dfe4ef]">
              ...
              {dimensionRows.length > 0 ? (
                ...
                <div className="px-4 py-12 text-center text-sm font-semibold text-[#747b8a]">
                  {extracting ? 'Reading the file...' : 'Add a takeoff item to start.'}
                </div>
              )}
            </section>
          </div>
