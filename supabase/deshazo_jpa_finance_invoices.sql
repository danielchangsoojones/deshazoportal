create table if not exists public.deshazo_jpa_finance_invoices (
  ...
  constraint deshazo_jpa_finance_revenue_nonnegative check (parts_revenue >= 0 and service_revenue >= 0)
);
...
grant select on table public.deshazo_jpa_finance_invoices to authenticated;
